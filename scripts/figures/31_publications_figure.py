...
=========================
Generate publication-quality PDF figures for CCN submission.

Figure A: Gain vs Loss sensitivity trajectory in vmPFC (from script 30 data)
Figure B: Comprehensive brain map (overall loss, loss time-on-task, gain time-on-task)

运行方式：conda activate narps && python 31_publication_figures.py
前提：先跑完 30_gain_trajectory.py（for Figure A）
      fatigue_neural_results/ 和 group_level_results/ 存在（for Figure B）
"""
...
# ============================================================
# FIGURE A: Gain vs Loss vmPFC Trajectory
# ============================================================

print("=" * 60)
print("Figure A: Gain vs Loss vmPFC Trajectory")
print("=" * 60)

data_file = 'gain_trajectory_results/trajectory_vmPFC.csv'
if os.path.exists(data_file):
    ...
    print(f"  Loss: r={r_loss:.3f}, p={p_loss:.4f}")
    print(f"  Gain: r={r_gain:.3f}, p={p_gain:.4f}")
    ...
    print("  保存: gain_vs_loss_vmPFC.pdf / .png")
    plt.close()
else:
    print(f"  ⚠️  {data_file} 不存在。请先跑 30_gain_trajectory.py")

# ============================================================
# FIGURE B: Comprehensive Brain Maps (PDF)
# ============================================================

print(f"\n{'=' * 60}")
print("Figure B: Comprehensive Brain Maps")
print("=" * 60)
...
try:
    ...
    loss_overall_path = os.path.join(group_dir, 'group_loss_zmap.nii.gz')
    loss_fatigue_path = os.path.join(fatigue_dir, 'group_loss_fatigue_zmap.nii.gz')
    gain_fatigue_path = os.path.join(fatigue_dir, 'group_gain_fatigue_zmap.nii.gz')

    missing = []
    for p, name in [(loss_overall_path, 'Overall loss'),
                    (loss_fatigue_path, 'Loss fatigue'),
                    (gain_fatigue_path, 'Gain fatigue')]:
    ...
    if missing:
        ...
        z_loss_overall = image.load_img(loss_overall_path)
        z_loss_fatigue = image.load_img(loss_fatigue_path)
        z_gain_fatigue = image.load_img(gain_fatigue_path)

        cut_coords = [-14, -4, 6, 14, 22, 32, 42, 54]

        fig, axes = plt.subplots(3, 1, figsize=(14, 11))

        # Panel A: Overall Loss
        plotting.plot_stat_map(
            z_loss_overall, threshold=2.3,
            title='(A)  Overall Loss Effect (all runs, $z_{\\mathrm{stat}}$ > 2.3)',
            display_mode='z', cut_coords=cut_coords,
            axes=axes[0], colorbar=True, annotate=True,
        )

        # Panel B: Loss Time-on-Task
        plotting.plot_stat_map(
            z_loss_fatigue, threshold=2.0,
            title='(B)  Loss Time-on-Task Change (Late $-$ Early, $z_{\\mathrm{stat}}$ > 2.0)',
            display_mode='z', cut_coords=cut_coords,
            axes=axes[1], colorbar=True, annotate=True,
        )

        # Panel C: Gain Time-on-Task
        plotting.plot_stat_map(
            z_gain_fatigue, threshold=2.0,
            title='(C)  Gain Time-on-Task Change (Late $-$ Early, $z_{\\mathrm{stat}}$ > 2.0)',
            display_mode='z', cut_coords=cut_coords,
            axes=axes[2], colorbar=True, annotate=True,
        )
        ...
        fig.savefig(os.path.join(output_dir, 'fig3c_comprehensive.png'),
                    format='png', bbox_inches='tight', pad_inches=0.15, dpi=200)
        print("  保存: fig3c_comprehensive.pdf / .png")
        plt.close()

except ImportError:
    print("  ⚠️  Nilearn not available. Skipping brain maps.")
...
print(f"\n{'=' * 60}")
print("所有图片保存在 paper_figures/")
print("=" * 60)
print("\n上传到 Overleaf 时使用 PDF 版本:")
print("  \\includegraphics[width=0.85\\textwidth]{paper_figures/gain_vs_loss_vmPFC.pdf}")
print("  \\includegraphics[width=0.95\\textwidth]{paper_figures/fig3c_comprehensive.pdf}")